...
def apply_llm_profile(
    llm_profile_json: Path,
    output_dir: Path,
    source_resume_md: Path | None = None,
    skip_rewrites: bool = False,
) -> dict[str, str]:
    payload = read_json(llm_profile_json)
    ...
    outputs = {
        ...
        "llm_profile_json": str(canonical_llm_json),
        "risk_report": str(risks_md),
        "source_markdown": str(source_resume_md or profile.get("markdown_path", "")),
    }
    if not skip_rewrites:
        _, rewrite_outputs = generate_resume_rewrite_suggestions(profile_json, output_dir=output_dir)
        outputs.update(
            {
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("llm_profile_json", help="Path to the LLM-generated profile JSON.")
    ...
    parser.add_argument("--skip-rewrites", action="store_true", help="Do not regenerate resume rewrite suggestions.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    outputs = apply_llm_profile(
        Path(args.llm_profile_json).resolve(),
